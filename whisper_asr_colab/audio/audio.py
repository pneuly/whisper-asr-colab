...
class Audio:
    ...
    def __init__(self, source: str | T, **kwargs: Any):
    ...
    def end_time(self) -> float:
        if isinstance(self._rawdata, np.ndarray):
            return (
                self.end_frame if self.end_frame else len(self._rawdata)
            ) / self.sampling_rate
        else:
            return 0.0
    ...
    def _load_audio(self) -> np.ndarray:
        ...
        logger.info(
            f"Loading audio file {self.local_file_path} ({os.path.getsize(self.local_file_path) / 1000000:.02f}MB)"
        )
        # Check if the uploading is finished.
        if self.verify_upload:
            logger.info(
                f"Checking if uploading {self.local_file_path} is still uploading."
            )
            uploading_flag, filesize1, filesize2, wait_time = is_uploading(
                self.local_file_path
            )
            if uploading_flag:
                logger.info(
                    f"File size increase is detected in {wait_time:.02f} seconds."
                )
                ...
                if "IPython" in sys.modules:
                    ...
                    display.display(
                        display.HTML(
                            f'<div style="color: red; font-size:large; font-weight: bold;">⚠️ {message}</div>'
                        )
                    )
                uploading_flag = True
                while uploading_flag:
                    uploading_flag, filesize1, filesize2, wait_time = is_uploading(
                        self.local_file_path, lambda x=10: time.sleep(x)
                    )
                    logger.info(
                        f"Uploaded size: {filesize2 / 1000000}MB ({(filesize2 - filesize1) / (wait_time * 1000):.01f} kB/s)"
                    )
            else:
                logger.info(
                    f"No file size increase is detected in {wait_time:.02f} seconds."
                )
        _rawdata = decode_audio(self.local_file_path, self.sampling_rate)
        if not isinstance(_rawdata, np.ndarray):
            raise ValueError(
                "Failed to get audio data. Check if url or file path is correctly set."
            )
    ...
    def set_silence_skip(
        self, threshold: float = 0.1, min_silence_duration: Union[int, float] = 5
    ) -> Tuple[Union[int, None], Union[int, None]]:
        """Set start_frame and end_frame based on silence detection"""
        if not isinstance(self.ndarray, np.ndarray):
            raise ValueError(
                "Cannot get self.ndarray as ndarray. Set url or file path to Audio instance."
            )
        ...
        if (not self.start_frame) and (leading > min_frame_size):
            logger.info(
                f"Leading silence detected. Skipping {leading / self.sampling_rate} seconds."
            )
            self.start_frame = int(leading)
        frame_size = len(self.ndarray)
        if (
            (not self.end_frame)
            and (trailing < frame_size)
            and (trailing + min_frame_size < frame_size)
        ):
            trailing_sec = (frame_size - trailing) / self.sampling_rate
            logger.info(
                f"Trailing silence detected. Skipping the last {trailing_sec} seconds."
            )
    ...
    def get_time_slice(
        ...
        end_sec: Union[int, float, None] = None,
    ) -> np.ndarray:
