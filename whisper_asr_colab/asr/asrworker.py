...
from typing import TYPE_CHECKING, BinaryIO, DefaultDict, Union
...
class ASRWorker:
    ...
    model: FasterWhisperModel | None = None
    asr_segments: SpeakerSegmentList | None = None
    transcribe_args: DefaultDict[str, str | int | bool]

    def __init__(
        self,
        audio: Audio,
        load_model_args: dict[str, str | int | bool] | None = None,
        transcribe_args: dict[str, str | int | bool] | None = None,
    ):
    ...
    def run(self) -> dict[str, str]:
        ...
        self.asr_segments, _ = faster_whisper_transcribe(
            audio=self.audio.ndarray, model=self.model, **self.transcribe_args
        )
        # outfilename = datetime.now().strftime("%Y%m%d_%H%M%S")
        outfiles = self.asr_segments.write_asr_result(
            self.audio.local_file_path
        )  # add timestamp offset if needed
        del self.model
