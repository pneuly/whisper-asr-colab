...
class SpeakerSegmentList(List[SpeakerSegment]):
    ...
    def assign_speakers(
        ...
        for asr_seg in self:
            while i <= dia_segments_size:
                ...
                logger.debug(
                    "[assign_speakers] asr_seg: %s, dia_seg: %s", asr_seg, dia_seg
                )
    ...
    def write_asr_result(
        ...
        with (
            open(asr_result_file, "w", encoding="utf-8") as fh_text,
            open(asr_result_file_timestamped, "w", encoding="utf-8") as fh_timestamped,
        ):
            for speaker_seg in self:
                fh_text.write(
                    speaker_seg.to_str(with_timestamp=False, with_speaker=False) + "\n"
                )
                fh_timestamped.write(
                    speaker_seg.to_str(timestamp_offset, with_speaker=False) + "\n"
                )
        return {
            "asr_result_file": asr_result_file,
